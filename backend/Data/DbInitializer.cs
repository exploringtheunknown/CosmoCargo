...
public static class DbInitializer
{
    ...
    private static readonly int TotalCustomers = Random.Shared.Next(200_000, 300_000);
    private static readonly int TotalPilots = Random.Shared.Next(100_000, 200_000);
    private static readonly int TotalAdmins = Random.Shared.Next(50_000, 100_000);
    private static readonly int TotalShipments = Random.Shared.Next(6_000_000, 8_000_000);

    private static readonly ConcurrentDictionary<string, byte> _takenEmails = new();
    ...
    private static async Task SeedDemoUsers(IServiceProvider serviceProvider)
    {
        ...
        var demoUsers = new List<User>
        {
            new()
            {
                ...
                PasswordHash = Crypto.HashPassword("mKv2P8dXrL9F"),
            ...
            new()
            {
                ...
                PasswordHash = Crypto.HashPassword("zH7yB3tR5wQ9s"),
                Role = UserRole.Pilot,
                Experience = null,
                IsActive = null,
                CreatedAt = DateTime.UtcNow.AddDays(-random.Next(0, 365))
            ...
            new()
            {
                ...
                PasswordHash = Crypto.HashPassword("eT4xD6cV2gN8p"),
                Role = UserRole.Admin,
                Experience = null,
                IsActive = null,
                CreatedAt = DateTime.UtcNow.AddDays(-random.Next(0, 365))
        ...
        await Parallel.ForEachAsync(Enumerable.Range(0, batches), options, async (batch, ct) =>
        {
            ...
            for (var i = 0; i < batchSize; i++)
            {
                ...
                users[i] = new User
                {
                    ...
                    Email = emails[i],
                    PasswordHash = Crypto.HashPassword($"Customer{userNumber}"),
                    Role = UserRole.Customer,
        ...
        await Parallel.ForEachAsync(Enumerable.Range(0, batches), options, async (batch, ct) =>
        {
            ...
            for (var i = 0; i < batchSize; i++)
            {
                ...
                users[i] = new User
                {
                    ...
                    Name = $"{_firstNames[firstNameIndices[i]]} {_lastNames[lastNameIndices[i]]}",
                    Email = emails[i],
                    PasswordHash = Crypto.HashPassword($"Pilot{userNumber}"),
                    Role = UserRole.Pilot,
                    Experience = experience,
                    IsActive = isActiveFlags[i],
                    CreatedAt = createdAts[i]
        ...
        await Parallel.ForEachAsync(Enumerable.Range(0, batches), options, async (batch, ct) =>
        {
            ...
            for (var i = 0; i < batchSize; i++)
            {
                ...
                users[i] = new User
                {
                    ...
                    Name = $"{_firstNames[firstNameIndices[i]]} {_lastNames[lastNameIndices[i]]}",
                    Email = emails[i],
                    PasswordHash = Crypto.HashPassword($"Admin{userNumber}"),
                    Role = UserRole.Admin,
                    Experience = experience,
                    IsActive = isActiveFlags[i],
                    CreatedAt = createdAts[i]
            ...
            for (var i = 0; i < batchSize; i++)
            {
                ...
                shipments[i] = new Shipment
                {
                    ...
                    PilotId = data.status == ShipmentStatus.Assigned
                        ...
                        ? random.Next(100) < 5 ? DemoPilotId : pilotIds[random.Next(pilotIds.Count)]
                        : null,
