...
public static class ChaosEventEndpoints
{
    ...
    {
        var enabled = config.GetValue("ChaosEngine:Enabled", true);
        var interval = config.GetValue("ChaosEngine:IntervalSeconds", 60);
        ...
        ClaimsPrincipal user)
    {
        ...
        return Results.Ok(new { request.IntervalSeconds });
    }
    ...
    public static async Task<IResult> UpdateChaosEventDefinition(int id, ChaosEventDefinitionRequest req,
        AppDbContext db)
    {
    ...
    public static async Task<IResult> GetChaosLogRetention(AppSettingsService settings, ClaimsPrincipal user)
    {
        if (!IsAdmin(user)) return Results.Forbid();
        int days = await settings.GetIntValueAsync("ChaosLogRetentionDays", 30);
        return Results.Ok(new { days });
    }

    public class SetRetentionRequest { public int Days { get; set; } }

    public static async Task<IResult> SetChaosLogRetention(SetRetentionRequest req, AppSettingsService settings, ClaimsPrincipal user)
    {
        if (!IsAdmin(user)) return Results.Forbid();
        if (req.Days < 1 || req.Days > 365) return Results.BadRequest("Days must be between 1 and 365.");
        await settings.SetIntValueAsync("ChaosLogRetentionDays", req.Days);
        return Results.Ok(new { days = req.Days });
    }

    public static async Task<IResult> ManualChaosLogCleanup(AppDbContext db, ClaimsPrincipal user)
    {
        if (!IsAdmin(user)) return Results.Forbid();
        var cutoff = DateTime.UtcNow.AddMinutes(-10);
        var oldLogs = db.ChaosEventLogs.Where(l => l.Timestamp < cutoff);
        db.ChaosEventLogs.RemoveRange(oldLogs);
        await db.SaveChangesAsync();
        return Results.Ok(new { message = "Old chaos logs deleted (except last 10 minutes)." });
    }
    ...
    public static RouteGroupBuilder MapChaosEventEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/log-retention", GetChaosLogRetention).RequireAuthorization("Admin");
        group.MapPost("/log-retention", SetChaosLogRetention).RequireAuthorization("Admin");
        group.MapPost("/manual-log-cleanup", ManualChaosLogCleanup).RequireAuthorization("Admin");
        return group;
    }
    ...
    public class ChaosEventDefinitionRequest
    {
        ...
        [Required] [Range(0.01, 1000)] public double Weight { get; set; }
...
{
    [Range(1, 86400)] public int IntervalSeconds { get; set; }
}
