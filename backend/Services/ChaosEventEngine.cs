...
namespace CosmoCargo.Services
{
    ...
    public class ChaosEventEngine
    {
        ...
        public async Task<(ChaosEventDefinition? SelectedEvent, ChaosEventLog? LogEntry)> SelectAndApplyChaosEventToShipmentAsync(Shipment shipment)
        {
            ...
            string impactDetails = string.Empty;
            ...
            switch (selected.Name)
            {
                ...
                case "PirateAttack":
                    shipment.Status = ShipmentStatus.InTransit;
                    shipment.UpdatedAt = now;
                    impactDetails = "Shipment attacked by pirates, delayed and flagged for review. Status set to InTransit.";
                    break;
                case "SolarFlare":
                    shipment.Status = ShipmentStatus.InTransit;
                    shipment.UpdatedAt = now;
